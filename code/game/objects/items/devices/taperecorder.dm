...
/obj/item/device/taperecorder/state
	icon_state = "tape2recorderidle"

/obj/item/device/taperecorder/state/hear_talk(mob/living/M as mob, msg, var/verb="says", datum/language/speaking=null)
	if(recording)
		timestamp += timerecorded
...
/obj/item/device/taperecorder/state/attackby(obj/item/weapon/W as obj, mob/user as mob)
	..()
	if(istype(W, /obj/item/weapon/card/emag))
		if(emagged == 0)
			emagged = 1
...
/obj/item/device/taperecorder/state/explode()
	var/turf/T = get_turf(loc)
	if(ismob(loc))
		var/mob/M = loc
		M << "<span class='danger'>\The [src] explodes!</span>"
...
/obj/item/device/taperecorder/state/record()
	set name = "Start Recording"
	set category = "Object"
	...
	if(emagged == 1)
		usr << "\red The tape recorder makes a scratchy noise."
		return
	icon_state = "tape2recorderrecording"
	if(timerecorded < 3600 && playing == 0)
		usr << "<span class='notice'>Recording started.</span>"
		recording = 1
		timestamp+= timerecorded
...
/obj/item/device/taperecorder/state/stop()
	set name = "Stop"
	set category = "Object"
...
/obj/item/device/taperecorder/state/clear_memory()
	set name = "Clear Memory"
	set category = "Object"
...
/obj/item/device/taperecorder/state/playback_memory()
	set name = "Playback Memory"
	set category = "Object"
...
/obj/item/device/taperecorder/state/print_transcript()
	set name = "Print Transcript"
	set category = "Object"
...
/obj/item/device/taperecorder/state/attack_self(mob/user)
	if(recording == 0 && playing == 0)
		if(usr.stat)
			return
		if(emagged == 1)
